...
function print_otvety($family1, $name1)
{
    ...
    echo('<table border="2" ><tr><th rowspan="2" class="head_table">Сотрудник</th> <th colspan="19" class="head_table">Ответы</th></tr>');
    $link = connect();
    //$login= "SELECT * FROM `lama` WHERE date_test LIKE '".$_POST['place']."' AND `place` = '".$name_place."'";
    ...
    echo('<tr><th rowspan="2" >'. $_POST['family1'] .' '. $_POST['name1'] .'</th> </tr>');
    $link = connect();
    //$login= "SELECT * FROM `lama` WHERE date_test LIKE '".$_POST['place']."' AND `place` = '".$name_place."'";
    $login = "SELECT * FROM `motiv` WHERE family = '" . $_POST['family1'] . "'";
    ...
    while ($row = $res->fetch_assoc()) {
        ...
        while ($i < 10) {
            //
            echo('<th> ' . $row['quest0' . $i . ''] . ' </th>');
            $i = $i + 1;
        }
        while ($i > 9 && $i < 19) {
            echo('<th>' .  $row['quest' . $i . '']  . ' </th>');
            $i = $i + 1;
        }
